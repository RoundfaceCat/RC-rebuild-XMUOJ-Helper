...
def main():
    ...
    while True:
        ...
        if action == "Solve a Single Problem":
            ...
            saved_contests = config.get("contest_passwords", {})
            choices = ["➕ Enter a New Contest"]
            if saved_contests:
                choices.extend([f"🏆 Saved Contest: {cid}" for cid in saved_contests.keys()])
                choices.append("🗑️  Delete a Saved Contest")
            choices.append("⬅️  Back")
            
            contest_choice = questionary.select("Select Contest:", choices=choices).ask()
            if not contest_choice or contest_choice == "⬅️  Back":
                continue
                
            if contest_choice == "🗑️  Delete a Saved Contest":
                del_choices = [f"Contest {cid}" for cid in saved_contests.keys()] + ["Cancel"]
                to_del = questionary.select("Select a contest to delete:", choices=del_choices).ask()
                if to_del and to_del != "Cancel":
                    del_cid = to_del.split(" ")[1]
                    if del_cid in config["contest_passwords"]:
                        del config["contest_passwords"][del_cid]
                        with open('config.json', 'w', encoding='utf-8') as f:
                            json.dump(config, f, indent=2)
                        console.print(f"[green]✅ Deleted saved contest {del_cid}[/green]")
                continue
                
            if contest_choice == "➕ Enter a New Contest":
                ...
                contest_id = contest_choice.replace("🏆 Saved Contest: ", "")
                password = saved_contests.get(contest_id, "")
            
            if contest_choice == "➕ Enter a New Contest" or password != saved_contests.get(contest_id, ""):
